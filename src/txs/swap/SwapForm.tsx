...
const SwapForm = () => {
	...
	const getOptions = (key: "offerAsset" | "askAsset") => {
		...
		const getOptionList = (list: TokenItemWithBalance[]) =>
			list.map((item) => {
				...
				const muted = {
					offerAsset: !!askAsset && !getIsSwapAvailable({ offerAsset: value, askAsset }),
					askAsset: !!offerAsset && !getIsSwapAvailable({ offerAsset, askAsset: value }),
				}[key];
	...
	const assets = useMemo(() => ({ offerAsset, askAsset }), [offerAsset, askAsset]);
	...
	const createTx = useCallback(
		(values: TxValues) => {
			const { mode, offerAsset, askAsset, input, slippageInput, ratio } = values;
			if (!(mode && input && offerAsset && askAsset && slippageInput && ratio)) return;
	...
	const tx = {
		...
		queryKeys: [offerAsset, askAsset]
			...
			.map((token) => [queryKey.wasm.contractQuery, token, { balance: address }]),
	};
		...
		return (
			<section className={styles.modes}>
				{availableSwapModes.map((key) => {
					...
					return (
						<RadioButton {...register("mode")} value={key} checked={checked} key={key}>
							{key}
						</RadioButton>
					);
				})}
