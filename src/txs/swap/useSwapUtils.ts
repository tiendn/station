...
import { Coin, Coins, MsgExecuteContract, MsgSwap } from "@terra-money/feather.js";
...
const useSwapUtils = () => {
	...
	/* determine swap mode */
	const getIsOnchainAvailable = useCallback(({ offerAsset, askAsset }: SwapAssets) => {
	...
	const getSwapMode = useCallback(
		(assets: SwapAssets) => {
			...
			if (isDenomLuna(askAsset)) {
				return getIsTerraswapAvailable(assets) ? SwapMode.TERRASWAP : SwapMode.ROUTESWAP;
			}
	...
	const getTerraswapParams = useCallback(
		(params: SwapParams, dex: Dex) => {
			...
			/* execute */
			const swap = belief_price && max_spread ? { belief_price, max_spread } : {};
	...
	const simulateTerraswap = async (params: SwapParams, dex: Dex = "terraswap") => {
		const mode = {
		...
		if (pair.type === "stable") {
			const { return_amount: value, commission_amount } = await lcd.wasm.contractQuery<{
	...
	const simulateAstroport = (params: SwapParams) => simulateTerraswap(params, "astroport");
	...
	const getMsgsFunction = useCallback(
		(mode: SwapMode) => {
			const getMsgs = {
				[SwapMode.ONCHAIN]: (params: SwapParams) => getOnchainParams(params).msgs,
				...
				[SwapMode.ROUTESWAP]: (params: SwapParams) => getRouteswapParams(params).msgs,
			};

			return getMsgs[mode];
		},
		[getOnchainParams, getTerraswapParams, getAstroportParams, getRouteswapParams]
	);
