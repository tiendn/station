...
import { readAmount, readDenom, toAmount } from "terra-classic-utils";
...
const SubmitProposalForm = ({ communityPool, minDeposit }: Props) => {
	...
	const { register, trigger, control, watch, setValue, handleSubmit, getValues } = form;
	...
	const shouldAppendChange = values.type === ProposalType.PARAMS && !values.changes.length;
		...
		({ input, title, description, ...values }: TxValues) => {
			...
			const getContent = () => {
				if (values.type === ProposalType.SPEND) {
					...
					return new CommunityPoolSpendProposal(title, description, recipient, coins);
				}
		...
		if (values.type === ProposalType.SPEND) {
			...
			return (
				<>
					<FormItem
						label={t("Recipient")}
						error={"spend" in errors ? errors.spend?.recipient?.message : undefined}
					>
						<Input
	...
	const agoraURL = isClassic ? "commonwealth.im/terra-classic" : "commonwealth.im/terra";
				...
				<Form onSubmit={handleSubmit(submit.fn)}>
					<Grid gap={4}>
						<FormHelp>
							...
							<ExternalLink href={`https://${agoraURL}`}>{agoraURL}</ExternalLink>
						</FormHelp>
						<FormWarning>
							{t(
