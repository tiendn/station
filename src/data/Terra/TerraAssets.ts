...
export const useCW20Whitelist = (disabled = false) => {
	return useTerraAssetsByNetwork<CW20Whitelist>("cw20/tokens.json", disabled, (data) =>
...
export type ContractNames = "assertLimitOrder" | "routeswap" | "tnsRegistry" | "tnsReverseRecord";
...
export const shuffleByProtocol = <T extends CW20Whitelist | CW721Whitelist>(array: T) => {
	const shuffledPair = shuffle(
		toPairs(groupBy(([, { protocol, name }]) => protocol ?? name, toPairs(array)))
	);
