...
const Component = ({ whitelist, keyword }: Props) => {
	...
	const added = {
		...
		cw20: cw20.list.reduce<AddedCW20>((acc, item) => ({ ...acc, [item.token]: item }), {}),
	};
	...
	// if not listed
	const { data: tokenInfo, ...state } = useTokenInfoCW20(!listedItem ? keyword : "");
	...
	return <TokenList {...state} {...manage} results={results} renderTokenItem={renderTokenItem} />;
};
